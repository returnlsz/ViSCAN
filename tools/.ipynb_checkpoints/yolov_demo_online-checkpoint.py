...
class Predictor(object):
    ...
    def inference(self, img, other_result=[]):
        ...
        # print("outputs:shape:value:"+str(len(outputs)))
        # print("res_dic:shape:value:"+str(len(res_dic['cls_scores'])))
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    for frame_num, frame in enumerate(frames):
        print("video_len:"+str(len(frames)))
        # 以下三者都是当前图片
        tmp_imgs = []
        img = frame
        tmp_imgs.append(img)
        ...
        # other_result包含31张图片的信息，每张图片包含5类信息
        # if other_result != []:
        #     print("other_res_cls_scores:"+str(len(other_result['reg_scores'])))
        pred_result, res_dict = predictor.inference(imgs, other_result)
        # res_dict['cls_scores'] length = 30 , tmp_imgs length = 1
        N = int(res_dict['cls_scores'].shape[0] / len(tmp_imgs))
        
        # print("value1:"+str(res_dict['cls_scores'].shape[0]))
        # print("value2:"+str(len(tmp_imgs)))
        # print("N_value:"+str(N))
        
        print("res_dict_cls_feature_len:"+str(len(res_dict['boxes'][0])))
