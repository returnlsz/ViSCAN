...
from yolox.models.post_trans import MSA_yolov,MSA_yolov2
...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        self.trans = MSA_yolov2(dim=self.width, out_dim=4 * self.width, num_heads=heads, attn_drop=drop)
    ...
    def forward(self, xin, labels=None, imgs=None, nms_thresh=0.5):
        ...
        for k, (cls_conv, cls_conv2, reg_conv, reg_conv2,stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                origin_nms_features.append(cls_feat)
                origin_nms_regf.append(reg_feat)
            else:
        ...
        # ------------------------------------------------------------------------------------------
        # 新增了两个原始特征
        reg_feat_flatten2 = torch.cat(  # 16x6804x256                      #reg_feat
            [x.flatten(start_dim=2) for x in origin_nms_regf], dim=2
        ).permute(0, 2, 1)
        cls_feat_flatten2 = torch.cat(  # 16x6804x256
            [x.flatten(start_dim=2) for x in origin_nms_features], dim=2  # cls_feat2
        ).permute(0, 2, 1)  # [b,features,channels]
        # ------------------------------------------------------------------------------------------
        ...
        # cls
        features_cls, features_reg2, cls_scores, fg_scores = self.find_feature_score(cls_feat_flatten, pred_idx,
                                                                                    reg_feat_flatten2, imgs,
                                                                                    pred_result)
        # reg
        features_cls2, features_reg, cls_scores, fg_scores = self.find_feature_score(cls_feat_flatten2, pred_idx,
        ...
        features_reg2 = features_reg2.unsqueeze(0)  # 1x240x256      240=30*batchsize(8)
        features_cls2 = features_cls2.unsqueeze(0)  # 1x240x256
        ...
        if self.use_score:
            ...
            trans_cls, trans_reg = self.trans(features_cls, features_reg,features_cls2, features_reg2, cls_scores, fg_scores,
                                              sim_thresh=self.sim_thresh,
                                              ave=self.ave, use_mask=self.use_mask)
        else:
            # trans_cls, trans_reg = self.trans(features_cls, features_reg, None, None, sim_thresh=self.sim_thresh,
            #                                   ave=self.ave)
            trans_cls, trans_reg = self.trans(features_cls, features_reg,features_cls2, features_reg2, None, None, sim_thresh=self.sim_thresh,
                                              ave=self.ave)
