...
def my_postprocess(prediction, num_classes, fc_outputs, reg_fc_outputs,conf_thre=0.001, nms_thre=0.5):
    ...
    for i, detections in enumerate(prediction):
        ...
        # detections[:, :4] = reg_fc_outputs[i][:, :4]
        detections[:, 4] = reg_fc_outputs[i][:,0].sigmoid()
        detections[:, 5] = cls_conf[i].sigmoid()
        detections[:, 6] = cls_pred[i]
        tmp_cls_score = fc_outputs[i].sigmoid()
        tmp_reg_score = reg_fc_outputs[i][:,0].sigmoid().unsqueeze(-1)
        # reg
        reg_mask =  tmp_reg_score >= conf_thre

        cls_mask = tmp_cls_score >= conf_thre
        cls_mask = reg_mask & cls_mask
