...
class VIDDataset(torchDataset):
    ...
    def photo_to_sequence(self,dataset_path,lframe,gframe):
        ...
        Returns:
            split result
        '''
        # logger.info("photo_to_sequence start!")
        res = []
        dataset = np.load(dataset_path,allow_pickle=True).tolist()
        logger.info("current_working_path:\n{}".format(os.getcwd()))
        ...
        for element in dataset:
            ...
            if ele_len<lframe+gframe:
                ...
                if self.mode == 'random':
                    ...
                    split_num = int(ele_len / (lframe))
                    for i in range(split_num):
                        res.append(element[i * lframe:(i + 1) * lframe])
                elif self.mode == 'ga':
                    # 第一个区间
                    num1 = math.floor(lframe * 0.68)
                    # 第三个区间
                    num3 = ceil(lframe*0.05)
                    # 第二个区间
                    num2 = lframe - num3 - num1

                    split_num = int(ele_len / (num1))
                    all_local_frame = element[:split_num * num1]

                    for i in range(split_num):
                        num2_l = 0 if i * num1 - ceil(num1 / 2) < 0 else i * num1 - ceil(num1 / 2)
                        num2_r = ele_len if (i + 1) * num1 + ceil(num1 / 2) > ele_len else (i + 1) * num1 + ceil(num1 / 2)
                        num3_l = 0 if i * num1 - num1 < 0 else i * num1 - num1
                        num3_r = ele_len if (i + 1) * num1 + num1 > ele_len else (i + 1) * num1 + num1
                        num2_frame = random.sample(element[num2_l:i * num1] + element[(i + 1) * num1:num2_r], num2)
                        num3_frame = random.sample(element[num3_l:num2_l] + element[num2_r:num3_r], num3)
                        res.append(all_local_frame[i * num1:(i + 1) * num1]+num2_frame+num3_frame)
                else:
    ...
    def get_annotation(self,path,test_size):
        # logger.info("get_annotation start!")
        path = path.replace("Data","Annotations").replace("jpg","xml")
        ...
        anno_res = []
        for xmls in files:
            photoname = xmls.replace("Annotations","Data").replace("xml","jpg")
            ...
            for obj in objs:
                ...
                xmax = int(math.floor(float(obj.getElementsByTagName("xmax")[0].firstChild.data)))
                xmin = int(math.floor(float(obj.getElementsByTagName("xmin")[0].firstChild.data)))
                ymax = int(math.floor(float(obj.getElementsByTagName("ymax")[0].firstChild.data)))
                ymin = int(math.floor(float(obj.getElementsByTagName("ymin")[0].firstChild.data)))
                x1 = np.max((0,xmin))
                y1 = np.max((0,ymin))
